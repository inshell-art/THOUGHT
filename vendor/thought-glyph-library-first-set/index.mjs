...
export const loadFirstSetManifest = () => {
  manifestPromise ??= readJsonUrl(new URL("./manifest.json", import.meta.url));
  return manifestPromise;
};

export const listFirstSetFonts = async () => {
  const manifest = await loadFirstSetManifest();
  return manifest.fonts.map((font) => ({ ...font }));
};

export const loadFirstSetFont = async (slug) => {
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(slug)) {
    throw new Error(`invalid First Set slug: ${slug}`);
  }
  const manifest = await loadFirstSetManifest();
  const record = manifest.fonts.find((font) => font.slug === slug);
  if (!record) throw new Error(`unknown First Set font: ${slug}`);
  const font = await readJsonUrl(new URL(record.file, import.meta.url));
  if (font.family?.slug !== slug) throw new Error(`${slug} font payload has a mismatched family slug`);
  if (font.librarySet?.memberId !== record.memberId) {
    throw new Error(`${slug} font payload has a mismatched First Set member ID`);
  }
  return font;
};

export const loadAllFirstSetFonts = async () => {
  const manifest = await loadFirstSetManifest();
  return Promise.all(manifest.fonts.map(async (record) => ({
    record: { ...record },
    font: await loadFirstSetFont(record.slug)
  })));
};

export const renderFirstSetLine = (
  font,
  text,
  {
    fill = "#00ba00",
  ...
  if (!font || !Array.isArray(font.glyphs) || !font.metrics) {
    throw new Error("a First Set glyph manifest is required");
  }
  ...
  for (const character of text) {
    ...
    if (!glyph) {
      throw new Error(`unsupported First Set character U+${character.codePointAt(0).toString(16).toUpperCase().padStart(4, "0")}`);
    }
    if (glyph.d) {
      paths.push(`<path d="${glyph.d}" transform="translate(${cursor} 0)" fill-rule="${font.metrics.fillRule}"/>`);
    }
    cursor += advance;
  }
...
export const renderAllFirstSetLines = async (text, options = {}) => {
  const entries = await loadAllFirstSetFonts();
  return entries.map(({ record, font }) => ({
    slug: record.slug,
    name: record.name,
    memberId: record.memberId,
    svg: renderFirstSetLine(font, text, options)
  }));
};
