...
> Historical context: earlier sections record the V1-era pack. The active formal signing pack is generated by `scripts/write-thought-signing-os-pack.mjs` and deploys `ThoughtSpecRegistry`, `ThoughtSpecRegistryV2`, `ThoughtRenderer`, and release-bound `ThoughtNFT`. It requires an approved `release.manifest.json`; draft manifests are rejected.
...
- deploy `ThoughtNFT(pathNft, registry, renderer, protocolRegistry, protocolReleaseId)`
