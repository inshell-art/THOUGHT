...
> Archive: this book describes the V1 deploy pack. The active formal signing pack is generated by `scripts/write-thought-signing-os-pack.mjs` and deploys only `ThoughtSpecRegistry` and `ThoughtNFT` with `THOUGHT.v2.md`.
