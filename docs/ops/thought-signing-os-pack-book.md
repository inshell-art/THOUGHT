...
> Historical context: earlier generated packs predated the active V2 release binding and creation attestation. The active formal signing pack is generated by `scripts/write-thought-signing-os-pack.mjs` and deploys `ThoughtSpecRegistry`, `ThoughtSpecRegistryV2`, `ThoughtRenderer`, `CreationAttestationVerifier`, and release-bound `ThoughtNFT`. It requires an approved `release.manifest.json`; draft manifests are rejected.
...
- Registering the pinned `THOUGHT.v2.md` spec.
- Deploying `CreationAttestationVerifier` with Ledger ADMIN ownership and an explicit reviewed service-key authority address.
...
- approved protocol release id, manifest hash, manifest URI, renderer profile hash, and work profile hash
- creation-attestation profile id and initial public authority address
...
- protocol release id, manifest hash/URI, and profile hashes
- creation-attestation verifier owner and initial public authority address
...
- register `THOUGHT.v2.md` in registry using ADMIN Ledger signer
- deploy immutable `ThoughtRenderer`
- deploy `ThoughtSpecRegistryV2(ADMIN)` and register the approved exact manifest hash/URI with the Ledger ADMIN
- deploy `CreationAttestationVerifier(ADMIN, attestationAuthority)` without putting the authority private key in the pack
- deploy `ThoughtNFT(pathNft, registry, renderer, protocolRegistry, protocolReleaseId, creationAttestationVerifier)`
...
- `ThoughtNFT.thoughtRenderer()` equals deployed renderer
- `ThoughtNFT.protocolRegistry()` equals deployed protocol registry
- `ThoughtNFT.creationAttestationVerifier()` equals deployed verifier
- `ThoughtNFT.protocolReleaseId()` and `protocolManifestHash()` equal the approved release
- renderer/work-profile hash getters equal the manifest entries
- `ThoughtRenderer.RENDERER_ID_HASH()` equals the reviewed renderer ID hash
- registry owner equals ADMIN
- verifier profile id equals the approved creation-attestation profile
- verifier owner equals ADMIN, authority equals the reviewed public service-key address, authority epoch equals `1`, and verifier is not paused
- expected `THOUGHT.v2.md` spec is registered
...
- PATH NFT address
- ThoughtNFT address
- ThoughtRenderer address and renderer ID/hash
- registry address
- registry owner
