...
## Current Producer State and Remaining Rollout

The THOUGHT producer now has a deterministic local protocol-release builder and
offline checker. `npm run protocol:build` emits one exact manifest and embedded
bundle covering the creative spec, Agent schemas, attestation profile,
consumer-facing contract interface and ABIs, contract vectors, mint schema,
...
replace the protocol release, does not publish the contract/network component,
and does not yet construct the final umbrella release. Candidate/stable use of
that outer bridge still requires clean-source enforcement.

Remaining cross-repository rollout order:

1. Freeze and review the complete THOUGHT producer source and generated bytes.
2. Commit and tag one clean candidate; rerun every producer and parity gate.
3. Promote the exact approved candidate manifest bytes to
   `release.manifest.json` without rebuilding different bytes.
4. Publish the immutable protocol package and reviewed per-network contract
   deployment metadata.
5. Build the umbrella FE release that pins protocol, render, contract, source
   commit, tag, and hashes atomically.
6. Extend `inshell.art` sync to verify that umbrella release and write one
   immutable consumer lock.
7. Vendor/embed exact V2 artifacts without independent edits and serve the
   production Agent/mint flow from that lock.
8. Remove production dependence on independently maintained THOUGHT/V1 task or
   renderer copies; retain V1 only as an unreachable historical archive where
   required.
9. Add producer/consumer CI proving the embedded release, sealed Agent task,
   provenance, ABI, deployment, and onchain registry identify the same release
   and selected spec.
...
- `docs/agent/IN_SHELL_ART_V2_PROTOCOL_REFINEMENT_HANDOFF.md` describes current downstream integration requirements.
