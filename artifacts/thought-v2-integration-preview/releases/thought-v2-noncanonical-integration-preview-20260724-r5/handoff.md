...
Artifact ID: `thought-v2-noncanonical-integration-preview-20260724-r5`
...
Consume the immutable experimental artifact:
  ...
  artifact ID:
    thought-v2-noncanonical-integration-preview-20260724-r5
  pointer:
...
Both role fields pack rows upward from a fixed bottom, so the prompt’s final
glyph-row baseline is always 332.8 and the Agent’s is always 780.8. Do not
top-align, vertically center, or independently reflow either field.
...
so consumers fail closed instead of silently retaining the former prompt
top-alignment.
