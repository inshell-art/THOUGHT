...
Purpose: hand off the current THOUGHT V2 local render behavior to FE so the production frontend can adopt the current square agent block / prompt block artifact look.
...
The agent line renders as a square-corner speech block with a center tail. The prompt line renders as a square-corner block near the bottom.
...
agent bg height = 70
agent bg rx = 0
agent tail path = M 470 448 L 480 465 L 490 448 Z
agent clip x = 88
agent clip y = 378
