...
const BINARY_BG_MIN_DOT_RADIUS = 1;
const BINARY_BG_MAX_DOT_RADIUS = 18;
...
export const THOUGHT_V2_RENDER_CONTRACT = {
  ...
  binaryBackground: {
    ...
    cell: {
      ...
      minDotRadius: BINARY_BG_MIN_DOT_RADIUS,
      maxDotRadius: BINARY_BG_MAX_DOT_RADIUS,
    },
    fill: BINARY_BG_FILL,
...
const binaryBackgroundLayout = (
  ...
  if (bits.length === 0) {
    return { circles: [], columns: 0, rows: 0, cellSize: 0, originX: BINARY_BG_X, originY: BINARY_BG_Y, radius: 0 };
  }
  ...
  const radius = Math.max(
    BINARY_BG_MIN_DOT_RADIUS,
    Math.min(BINARY_BG_MAX_DOT_RADIUS, Math.floor(cellSize * BINARY_BG_DOT_RADIUS_RATIO)),
  );
  ...
  return { circles, columns, rows, cellSize, originX, originY, radius };
};

const binaryBackground = (promptLine: string, agentLine: string): string => {
  const { circles, columns, rows, cellSize, originX, originY, radius } = binaryBackgroundLayout(promptLine, agentLine);
  return [
    `<g id="binary-background" opacity="${BINARY_BG_OPACITY_ATTR}" fill="${BINARY_BG_FILL}" aria-label="UTF-8 binary background: prompt line bytes then agent line bytes; green circles are one bits and empty square cells are zero bits" data-grid-columns="${columns}" data-grid-rows="${rows}" data-cell-size="${cellSize}" data-origin-x="${originX}" data-origin-y="${originY}" data-dot-radius="${radius}" data-zero="empty">`,
    ...circles.map((circle) => `  <circle cx="${circle.cx}" cy="${circle.cy}" r="${circle.r}"/>`),
    `</g>`,
  ].join("\n");
};
