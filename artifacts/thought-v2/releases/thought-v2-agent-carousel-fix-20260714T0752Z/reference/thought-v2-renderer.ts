...
const binaryBackground = (promptLine: string, agentLine: string): string => {
  ...
  return `<g id="binary-background" opacity="1" fill="${FIELD_COLOR}" aria-label="Interleaved UTF-8 binary field: 512 prompt positions and 512 Agent positions; filled circles are one bits and hollow rings are zero bits" data-grid-columns="32" data-grid-rows="32" data-bit-capacity="1024" data-prompt-bit-positions="512" data-agent-bit-positions="512" data-one-cells="${oneCount}" data-zero-cells="${zeroCount}" data-rendered-cells="${oneCount + zeroCount}" data-cleared-cells="${BINARY_FIELD_BITS - oneCount - zeroCount}" data-pack="msb-first-128-bytes" data-cell-size="28" data-origin-x="32" data-origin-y="32" data-dot-radius="10" data-zero="hollow-circle"><defs><circle id="binary-one" r="10" fill="${FIELD_COLOR}"/><pattern id="binary-zero-pattern" x="32" y="32" width="28" height="28" patternUnits="userSpaceOnUse"><circle id="binary-zero" cx="14" cy="14" r="10" fill="none" stroke="${FIELD_COLOR}" stroke-width="1"/></pattern></defs><rect id="binary-zero-field" x="32" y="32" width="896" height="896" fill="url(#binary-zero-pattern)"/>${uses.join("")}<rect id="agent-text-clear" x="92" y="372" width="776" height="76" fill="${CANVAS_BG}"/><rect id="prompt-text-clear" x="148" y="820" width="664" height="48" fill="${CANVAS_BG}"/></g>`;
};
