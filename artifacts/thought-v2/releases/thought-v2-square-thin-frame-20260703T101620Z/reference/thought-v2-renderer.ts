...
const AGENT_BG = { x: 72, y: 365, width: 816, height: 96, radius: 0 };
const AGENT_CLIP = { x: 88, y: 365, width: 784, height: 96, radius: 0 };
const AGENT_TEXT_Y = 413;
const AGENT_TAIL_PATH = "M 470 461 L 480 478 L 490 461 Z";
const AGENT_FRAME_STROKE_WIDTH = 1;
const PROMPT_X = 480;
const PROMPT_TARGET_WIDTH = 628;
const PROMPT_BASE_FONT = 16;
const PROMPT_MIN_FONT = 16;
const PROMPT_BG = { x: 150, y: 868, width: 660, height: 44, radius: 0 };
...
export const buildThoughtV2Svg = ({
  ...
  return [
    ...
    `    <rect id="agent-line-bg" x="${AGENT_BG.x}" y="${AGENT_BG.y}" width="${AGENT_BG.width}" height="${AGENT_BG.height}" rx="${AGENT_BG.radius}" fill="${escapeXml(canvasBg)}" stroke="${escapeXml(agentBg)}" stroke-width="${AGENT_FRAME_STROKE_WIDTH}"/>`,
    `    <path id="agent-line-tail" d="${AGENT_TAIL_PATH}" fill="${escapeXml(canvasBg)}" stroke="${escapeXml(agentBg)}" stroke-width="${AGENT_FRAME_STROKE_WIDTH}"/>`,
