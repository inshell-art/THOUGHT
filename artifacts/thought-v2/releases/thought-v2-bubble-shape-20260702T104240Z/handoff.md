...
Purpose: hand off the current THOUGHT V2 local render behavior to FE so the production frontend can adopt the new rounded bubble / prompt pill artifact look.
...
The agent line renders as a rounded speech bubble with a center tail. The prompt line renders as a small rounded pill near the bottom.
...
agent bg height = 96
agent bg rx = 14
agent tail path = M 470 461 L 480 478 L 490 461 Z
agent clip x = 88
agent clip y = 365
agent clip width = 784
agent clip height = 96
agent text x = 480
agent text y = 413
```
...
9. Keep prompt line with the small rounded pill background unless product direction changes.
