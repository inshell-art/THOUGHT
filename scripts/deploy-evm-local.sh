...
THOUGHT_PROTOCOL_RELEASE_HASH="${THOUGHT_PROTOCOL_RELEASE_HASH:-$(node -p "JSON.parse(require('fs').readFileSync('$ROOT_DIR/protocol/CURRENT.json','utf8')).keccak256")}"
...
(
  ...
  forge create \
    ...
    --constructor-args "$PATH_NFT_ADDRESS" "$REGISTRY_ADDRESS" "$THOUGHT_PROTOCOL_RELEASE_HASH" >"$tmp_token"
)
