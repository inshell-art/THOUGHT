...
ADDRESSES_FILE="$EVM_DIR/addresses.v2.anvil.json"
RPC_URL="${RPC_URL:-http://127.0.0.1:8545}"
PRIVATE_KEY="${PRIVATE_KEY:-}"
PATH_NFT_ADDRESS="${PATH_NFT_V2_ADDRESS:-${PATH_NFT_ADDRESS:-}}"
...
if [[ -z "$PRIVATE_KEY" ]]; then
  echo "PRIVATE_KEY is required for local V2 deployment." >&2
  exit 1
fi

if [[ -z "$PATH_NFT_ADDRESS" ]]; then
  echo "PATH_NFT_V2_ADDRESS or PATH_NFT_ADDRESS is required." >&2
  echo "Deploy/reference PATH V2 first, then rerun with PATH_NFT_V2_ADDRESS=<PathNFTV2 address>." >&2
  exit 1
fi
...
if [[ "$REGISTERED_PAIR" != "true" ]]; then
  echo "Registered THOUGHT V2 spec ID/hash pair did not validate." >&2
  exit 1
fi
...
if [[ "$READBACK_HASH" != "$THOUGHT_SPEC_HASH" ]]; then
  echo "THOUGHT V2 spec readback hash mismatch." >&2
  exit 1
fi
...
(
  ...
  forge create \
    ...
    --private-key "$PRIVATE_KEY" \
    --json \
    src/ThoughtNFTV2.sol:ThoughtNFTV2 \
...
payload = {
    ...
    "pathNftV2": path_nft_address,
    "thoughtSpecRegistryV2": registry_address,
    "thoughtSpecRegistryV2Owner": registry_owner,
    "thoughtNftV2": token_address,
    "movement": "THOUGHT",
...
echo "ThoughtSpecRegistryV2: $REGISTRY_ADDRESS"
echo "ThoughtSpecRegistryV2 owner: $THOUGHT_REGISTRY_OWNER"
echo "ThoughtNFTV2:          $TOKEN_ADDRESS"
echo "PathNFTV2:             $PATH_NFT_ADDRESS"
if [[ "$CONFIGURE_PATH_MOVEMENT" == "1" ]]; then
