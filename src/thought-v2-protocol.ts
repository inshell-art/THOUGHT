...
export const RELEASE_ID_DOMAIN_TEXT = "INSHELL_THOUGHT_PROTOCOL_RELEASE" as const;
export const AGENT_IDENTITY_DOMAIN = id(AGENT_IDENTITY_DOMAIN_TEXT);
export const WORK_DOMAIN = id(WORK_DOMAIN_TEXT);
export const RELEASE_ID_DOMAIN = id(RELEASE_ID_DOMAIN_TEXT);
...
export const deriveThoughtTraits = (promptLine: string, agentLine: string): ThoughtTraits => {
  assertThoughtLine(promptLine, "prompt");
  assertThoughtLine(agentLine, "agent");
  const prompt = fitBinarySource64(promptLine);
  const agent = fitBinarySource64(agentLine);
  let promptWeight = 0;
  let agentWeight = 0;
  let bitDistance = 0;
  for (let index = 0; index < 64; index += 1) {
    promptWeight += popcount(prompt[index]!);
    agentWeight += popcount(agent[index]!);
    bitDistance += popcount(prompt[index]! ^ agent[index]!);
  }
  const loomWeight = promptWeight + agentWeight;
  return {
    promptBytes: strictUtf8Bytes(promptLine).length,
    agentBytes: strictUtf8Bytes(agentLine).length,
    promptWeight,
    agentWeight,
    loomWeight,
    bitDistance,
...
export const deriveProtocolReleaseId = (manifestHash: `0x${string}`): `0x${string}` => {
  if (!/^0x[0-9a-f]{64}$/.test(manifestHash)) throw new Error("manifest hash must be lowercase bytes32");
  return keccak256(
    abiCoder.encode(["bytes32", "bytes32"], [RELEASE_ID_DOMAIN, manifestHash]),
  ) as `0x${string}`;
};
