...
import {ThoughtSpecRegistryV1} from "../../legacy/ThoughtSpecRegistryV1.sol";
import {ColorFontV1, ColorFontV1Data} from "../../legacy/ColorFontV1.sol";
import {ThoughtNFTV1} from "../../legacy/ThoughtNFTV1.sol";
...
contract FakeColorFontV1Archive {
    string private _id;
...
contract ThoughtNFTV1Test {
    ...
    MockPathNFTV1 private path;
    ColorFontV1 private colorFont;
    ThoughtSpecRegistryV1 private registry;
    ThoughtNFTV1 private token;
    address private user;

    function setUp() public {
        user = vm.addr(USER_KEY);
        path = new MockPathNFTV1();
        colorFont = new ColorFontV1();
        registry = new ThoughtSpecRegistryV1(address(this));
        ...
        token = new ThoughtNFTV1(address(path), address(registry), address(colorFont));
    ...
    function testConstructorPinsDependenciesAndRejectsInvalidTargets() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.InvalidPathNft.selector));
        new ThoughtNFTV1(address(0), address(registry), address(colorFont));

        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.InvalidPathNft.selector));
        new ThoughtNFTV1(address(0x1234), address(registry), address(colorFont));

        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.InvalidThoughtSpecRegistry.selector));
        new ThoughtNFTV1(address(path), address(0), address(colorFont));

        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.InvalidThoughtSpecRegistry.selector));
        new ThoughtNFTV1(address(path), address(0x1234), address(colorFont));

        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.InvalidColorFont.selector));
        new ThoughtNFTV1(address(path), address(registry), address(0));

        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.InvalidColorFont.selector));
        new ThoughtNFTV1(address(path), address(registry), address(0x1234));

        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.InvalidColorFont.selector));
        new ThoughtNFTV1(address(path), address(registry), address(registry));

        FakeColorFontV1Archive wrongColorFontId = new FakeColorFontV1Archive("thought.colorfont.v1", "v1", ColorFontV1Data.hash());
        FakeColorFontV1Archive wrongColorFontVersion = new FakeColorFontV1Archive(ColorFontV1Data.id(), "v2", ColorFontV1Data.hash());
        FakeColorFontV1Archive wrongColorFontHash =
            new FakeColorFontV1Archive(ColorFontV1Data.id(), ColorFontV1Data.version(), bytes32(0));

        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.InvalidColorFont.selector));
        new ThoughtNFTV1(address(path), address(registry), address(wrongColorFontId));

        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.InvalidColorFont.selector));
        new ThoughtNFTV1(address(path), address(registry), address(wrongColorFontVersion));

        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.InvalidColorFont.selector));
        new ThoughtNFTV1(address(path), address(registry), address(wrongColorFontHash));
    }
    ...
    function testRegistryConstructorPinsOwnerAndRejectsZeroOwner() public {
        ThoughtSpecRegistryV1 ownedRegistry = new ThoughtSpecRegistryV1(user);
        require(ownedRegistry.owner() == user, "registry owner mismatch");

        vm.expectRevert(abi.encodeWithSelector(ThoughtSpecRegistryV1.OwnerZeroAddress.selector));
        new ThoughtSpecRegistryV1(address(0));

        vm.expectRevert(abi.encodeWithSelector(ThoughtSpecRegistryV1.NotOwner.selector));
    ...
    function testSpecDataValidation() public {
        vm.expectRevert(abi.encodeWithSelector(ThoughtSpecRegistryV1.EmptyThoughtSpec.selector));
        registry.registerThoughtSpec("THOUGHT.v2.md", "empty", "");

        bytes memory tooLarge = _bytesRepeat("S", registry.MAX_THOUGHT_SPEC_BYTES() + 1);
        vm.expectRevert(
            abi.encodeWithSelector(
                ThoughtSpecRegistryV1.ThoughtSpecTooLarge.selector,
                tooLarge.length,
    ...
    function testRenderThoughtSvgRejectsNonCanonicalText() public {
        vm.expectRevert(abi.encodeWithSelector(ThoughtNFTV1.NonCanonicalThoughtText.selector));
    ...
    function testWrongAndZeroThoughtSpecPairsRevert() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ThoughtNFTV1.InvalidThoughtSpecPair.selector, DEFAULT_SPEC_ID, bytes32(uint256(0xBEEF))
            )
        );
        token.mint(
            "HELLO",
        ...
        vm.prank(user);
        vm.expectRevert(
            abi.encodeWithSelector(ThoughtNFTV1.InvalidThoughtSpecPair.selector, bytes32(0), DEFAULT_SPEC_HASH)
        );
        token.mint(
            "WORLD",
            2,
        ...
        vm.prank(user);
        vm.expectRevert(
            abi.encodeWithSelector(ThoughtNFTV1.InvalidThoughtSpecPair.selector, DEFAULT_SPEC_ID, bytes32(0))
        );
        token.mint(
            "THIRD",
            3,
    ...
    function testOversizeTextReverts() public {
        ...
        vm.prank(user);
        vm.expectRevert(
            abi.encodeWithSelector(ThoughtNFTV1.ThoughtTextTooLarge.selector, bytes(text).length, token.MAX_TEXT_BYTES())
        );
    ...
    function _assertOversizeProvenanceReverts(string memory provenance, uint256 pathId) private {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ThoughtNFTV1.ProvenanceTooLarge.selector, bytes(provenance).length, token.MAX_PROVENANCE_BYTES()
            )
        );
