...
contract ThoughtNFTV1 {
        ...
        returns (bytes32 specId, bytes32 specHash, string memory specName, string memory ref)
    {
        ...
        (bool exists, string memory specName_, bytes32 registeredHash, string memory ref_,,,) =
            IThoughtSpecRegistryV1(thoughtSpecRegistry).thoughtSpecMeta(specId);
    ...
    function mint(
        ...
        if (
            providedThoughtSpecId == bytes32(0) || providedThoughtSpecHash == bytes32(0)
                || !IThoughtSpecRegistryV1(thoughtSpecRegistry).isRegisteredThoughtSpec(
        ...
        uint32 pathSerial =
            IPathNFTV1(pathNft).consumeUnit(pathId, PATH_MOVEMENT_THOUGHT, msg.sender, deadline, pathSignature);
        ...
        returns (bool)
    {
        ...
        try IERC721ReceiverV1(to).onERC721Received(msg.sender, from, tokenId, data) returns (bytes4 retval) {
            return retval == IERC721ReceiverV1.onERC721Received.selector;
        } catch {
            return false;
        }
    }
